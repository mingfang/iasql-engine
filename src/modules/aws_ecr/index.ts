...
class RepositoryImageMapper extends MapperBase<RepositoryImage> {
  ...
  async repositoryImageMapper(image: ImageDetail, ctx: Context, type: string, region?: string) {
    ...
    if (!image.imageDigest || !image.imageTags) {
      throw new Error('Invalid repository image');
    }
    out.imageDigest = image.imageDigest;
    out.imageTag = image.imageTags[0] ?? '<untagged>';
    ...
    read: async (ctx: Context, id?: string) => {
      ...
      if (id) {
        ...
        for (const ri of images) {
          out.push(await this.repositoryImageMapper(ri, ctx, 'private', ri.region));
        }
        for (const ri of publicImages) {
          out.push(await this.repositoryImageMapper(ri, ctx, 'public', undefined));
        }
        return out;
      }
    },
